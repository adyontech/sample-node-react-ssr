module.exports = {
  ...
  checkDevice: function(req, res, next) {
    // check to see if the caller is a mobile device
    const isMobile = req.useragent && req.useragent.isMobile;
    const fromMobileDomain = req.hostname === "m.redbus.in";

    const fromDestopDomain = req.hostname === "www.redbus.in";

    if (isMobile && fromDestopDomain) {
      res.redirect("http://m.redbus.in" + req.url);
    }

    if(!isMobile && fromMobileDomain){
      res.redirect("http://www.redbus.in" + req.url);
    }
